...
const listeners = new Set<(state: SiglipModelState) => void>();
...
export function subscribeSiglipModelState(listener: (state: SiglipModelState) => void) {
...
export async function downloadSiglipModels() {
  return downloadSiglipVisionAndTokenizer();
}
...
export async function downloadSiglipTextModel() {
  ...
  try {
    ...
    const textFile = new File(modelDir, 'siglip2_text.pte');
    if (!textFile.exists) {
      const result = await File.downloadFileAsync(textUrl, textFile, {
        idempotent: true,
      });
      if (!result) {
...
export function unloadSiglipModels() {
  const siglip = createSigLIP();
  siglip.unloadModels();
}
